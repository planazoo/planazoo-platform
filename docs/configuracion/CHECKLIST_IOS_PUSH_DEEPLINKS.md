...
- **Invitación a plan (usuario registrado):** Tras crear la notificación in-app, la app llama a la Cloud Function **`sendInvitationPush`**. El payload incluye `type=invitation`, `planId` y `tab=participants`. Al **tocar el push**, la app abre el modal **aceptar/rechazar** (`InvitationResponseDialog`) para ese `planId` (no solo navega a Participantes). Lo mismo al tocar la invitación en la campana in-app. Requiere **`firebase deploy --only functions:sendInvitationPush`**.
